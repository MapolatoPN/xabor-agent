...
(function (global) {
  ...
  // ─── Punto ÚNICO de selección de producto ─────────────────────────────────
  //
  // Todos los canales de venta entran por aquí: mostrador / para llevar,
  // recoger, domicilio y mesa. La pregunta "¿este producto hay que
  // configurarlo?" se responde en UN solo lugar.
  //
  // Existe porque no estaba: la captura de Envíos (recoger y domicilio) tenía
  // su propio catálogo y su propio carrito, metía el producto directo y ni
  // siquiera conservaba `modificadores` al leer /api/menu. Un producto con
  // grupos —Chilaquiles Mixtos, y cualquier otro de cualquier negocio— entraba
  // sin preguntar nada: si sus grupos eran opcionales la cocina recibía una
  // comanda incompleta, y si eran obligatorios el backend rechazaba el pedido
  // con GRUPO_REQUERIDO y el operador no podía capturarlo. Mostrador y mesa sí
  // preguntaban, cada uno con su copia de la misma decisión. Tres canales, dos
  // implementaciones y un olvido: por eso la decisión vive ahora aquí y los
  // tres la llaman.
  //
  //   modo: 'wizard' → un grupo por pantalla (Restaurante/Mesas)
  //         cualquier otro valor → formulario completo (POS)
  //
  // Devuelve null si el operador cancela (NO se agrega nada) y, si no, la
  // línea lista para el carrito. Un producto sin grupos se devuelve directo,
  // sin abrir nada. Las reglas (requerido / mínimo / máximo) las impone el
  // configurador antes de resolver, y el servidor las vuelve a imponer al
  // crear el pedido: esto es para no dejar pedir algo inválido, no la
  // frontera de seguridad.
  async function elegirLinea(producto, opciones = {}) {
    const linea = {
    ...
    if (!tieneModificadores(producto)) return linea;
    const configurar = opciones.modo === 'wizard' ? abrirWizard : abrirModal;
    const r = await configurar(producto, opciones);
    if (!r) return null;
    return {
      ...linea,
      cantidad: r.cantidad,
  ...
  // Dos veces el mismo producto con modificadores distintos son DOS líneas.
  // La identidad de una línea es (producto, opciones elegidas), nunca el
  // producto solo: agrupar por producto_id fundía "Chilaquiles verdes" y
  // "Chilaquiles rojos" en una sola línea y la cocina perdía una de las dos.
  function firmaLinea(linea) {
    const mods = (linea.modificadores || [])
      .map(Number).filter(Number.isFinite).sort((a, b) => a - b).join(',');
    return `${linea.producto_id ?? linea.nombre}|${mods}`;
  }

  global.XaborModificadores = { tieneModificadores, abrirModal, abrirWizard, textoRegla, elegirLinea, firmaLinea };
})(window);
