  ...
  for (const { producto, mods, notas } of resueltos) {
    ...
    if (conFidelidad) {
      ...
      for (const span of mencionesTurno) {
        ...
        // El cliente abrevia ("grande" por "Grande 1 Litro"). Antes de acusar a
        // una mención de inexistente hay que aplicarle la MISMA tolerancia que
        // ya se le aplica al respaldo y al nombre del producto; si no, se le
        // niega al cliente algo que el negocio sí vende.
        const aprox = buscarOpcionPorMencion(grupos, span);
        if (aprox.estado === 'resuelto') { modificadores.push(aprox.modificador); continue; }
        if (aprox.estado === 'ambiguo') { ambiguos.push({ nombre: span, grupos: aprox.grupos }); continue; }
        mencionesNoResueltas.push({ codigo: RECHAZOS.MENCION_NO_RESUELTA, texto: span, producto: producto.nombre });
      }
    }

    const { faltantes, excedidos, inconsistentes } = validarCardinalidadGrupos(grupos, modificadores);

    // Una mención NO RESUELTA es, por definición, la que no casó con ningún
    // grupo: no se le asigna ninguno. Antes heredaba los grupos FALTANTES del
    // producto y salía "no tenemos mango en medida" — inventar la pertenencia
    // es exactamente lo que este camino existe para impedir. Lo que falta por
    // elegir se pregunta por su vía normal (`faltantes`), no colgado de ella.
...
export function mensajeBorradorParaCliente(resultado) {
  ...
  for (const p of prods) {
    for (const m of (p.mencionesNoResueltas || [])) {
      if (!porProducto.has(p.producto)) porProducto.set(p.producto, new Set());
      porProducto.get(p.producto).add(m.texto);
    }
  }
  if (invalidos.length || porProducto.size) {
    const partes = [];
    for (const [prod, textos] of porProducto) {
      partes.push(`no manejamos ${listar([...textos].map((t) => `"${t}"`))} en ${prod}`);
    }
