...
try {
  ...
  for (const file of ['069_compras_operativas.sql','070_compras_pagos_fondos.sql','071_compras_whatsapp.sql']) {
  ...
  await db.query('SELECT 1 FROM compras_whatsapp_autorizados LIMIT 0');
  await db.query('SELECT 1 FROM compras_whatsapp_tickets LIMIT 0');
  await db.query('COMMIT'); console.log('[compras] Migraciones 069, 070 y 071 verificadas.');
} catch(e) {
