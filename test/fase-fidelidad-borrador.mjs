...
await t('F9. la mención imposible tiene prioridad sobre el grupo requerido faltante', () => {
  assert.doesNotMatch(f1.texto, /falta saber/i,
    `no puede degradarse a "me falta saber líquido" cuando el cliente dijo algo que no existe: ${f1.texto}`);
});
...
await t('F2. una selección que el cliente nunca expresó NO se acepta (SELECCION_SIN_RESPALDO)', async () => {
  ...
  assert.match(r.texto, /no tenemos omega/i, `hay que decir la verdad sobre omega: ${r.texto}`);
...
await t('F3. un <PEDIDO_BORRADOR> vacío NO desactiva la extracción independiente', async () => {
  ...
  assert.match(r.texto, /no tenemos omega/i, `un marcador vacío no puede ser una vía de escape: ${r.texto}`);
