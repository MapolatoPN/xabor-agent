-- Rollback de la migración 021. Guarda: aborta si ya existe alguna fila
-- real en documentos o cotizaciones -- significaría que la función ya
-- se usó en producción y este down perdería esos datos.
-- Ejecutar con: psql "$CONN" -v ON_ERROR_STOP=1 -f 021_documentos_cotizaciones_down.sql
...
BEGIN
  ...
  IF v_documentos > 0 OR v_cotizaciones > 0 THEN
    RAISE EXCEPTION
      'Rollback 021 abortado: existen % documento(s) y % cotización(es) reales. Este down las eliminaría.',
